...
int main(int argc, char* argv[]) {
    ...
    char* match = (char*)calloc(len + 1, sizeof(char));   // '-', lower, UPPER
...
int game_loop(char* correctWord, char* input, int* guesses, char* match, 
        Dictionary* dict, int len, int max) {
    if (*guesses >= max) {
	return 3;       // Guesses exceeded max, game over
    }
	
    input_prompt(len, max, *guesses);     // Input prompt
    ...
    input = string_to_lower(input, len); // Lower case for comparison
    ...
    char* upperCorrectWord = string_to_upper(correctWord, len);
    int fullyCorrect = strcmp(match, upperCorrectWord);
    // Correct guess
 ...
 * string_to_upper()
 *
 * Convers an entire string s to uppercase
 */
char* string_to_upper(char* s, int len) {
    char* copy;
    strcpy(copy, s);
